@REQ_MCF-1069
Feature: GAMING PLATFORM overview page
  As a visitor
  I want to see a list of all GAMING PLATFORMS
  So I can get a general idea of the different types that exist
  And by scrolling through them find interesting ones

  @RULE_MCF-1082
  Rule: The GAMING PLATFORM list gets paginated when the total GAMING PLATFORM count exceeds the predefined limit

    @TEST_MCF-1084 @implemented
    Scenario: Expecting the pagination bar on the GAMING PLATFORM overview page to have the necessary navigational elements
      Given there are 205 "GAMING PLATFORM"s
      When the user visits page 2 of the "GAMING PLATFORM" overview page
      Then the pagination bar should contain a link to the previous page
      And the pagination bar should contain a link to the next page
      And the pagination bar should indicate that the user is currently on page 2
