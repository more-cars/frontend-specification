...
Feature: Canonical URLs
  ...
  Rule: All links to a node detail page use canonical URLs
    ...
    Scenario Outline: Expecting all links on the node overview pages to use canonical URLs
      When the user visits the "<node_type>" overview page
      Then each link in the "<node_type>" section should be a canonical URL
