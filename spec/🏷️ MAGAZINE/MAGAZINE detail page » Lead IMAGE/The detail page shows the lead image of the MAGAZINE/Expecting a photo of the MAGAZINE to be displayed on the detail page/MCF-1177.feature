@REQ_MCF-1175
Feature: MAGAZINE detail page » Lead IMAGE
  As a visitor
  I want the detail page to show a photo of the MAGAZINE
  So I can instantly see how it looks like
  Or to quickly verify that I am on the correct page

  @RULE_MCF-1176
  Rule: The detail page shows the lead image of the MAGAZINE

    @TEST_MCF-1177 @implemented
    Scenario: Expecting a photo of the MAGAZINE to be displayed on the detail page
      When the user visits the detail page of a "MAGAZINE"
      Then the page should contain a photo section
      And the photo section should contain an image of the "MAGAZINE"
